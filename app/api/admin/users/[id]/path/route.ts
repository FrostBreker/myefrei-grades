...
export async function PUT(request: NextRequest, {params}: { params: Promise<{ id: string }> }) {
    return instrumentApiRoute('admin/users/[id]/path/PUT', async () => {
        try {
            ...
            recordCustomEvent('AdminPathChange', {
                targetUserId: userId,
                cursus,
                filiere,
                groupe,
            ...
            if (error instanceof Error) noticeError(error, { route: 'admin/users/[id]/path/PUT' });
...
export async function GET(request: NextRequest, {params}: { params: Promise<{ id: string }> }) {
    return instrumentApiRoute('admin/users/[id]/path/GET', async () => {
        try {
            const isAuthorized = await requestAdminCheck();
            if (!isAuthorized) return;
            ...
            if (error instanceof Error) noticeError(error, { route: 'admin/users/[id]/path/GET' });
            return NextResponse.json({error: "Erreur serveur"}, {status: 500});
        }
    });
}
